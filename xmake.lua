...
package("echo")
    add_deps("cmake")
    set_sourcedir(path.join(os.projectdir(), "build/_deps/echo-src"))

    on_fetch(function (package)
        local sourcedir = package:sourcedir()
        if not os.isdir(sourcedir) then
            print("Fetching echo from git...")
            os.mkdir(path.directory(sourcedir))
            os.execv("git", {"clone", "--quiet", "--depth", "1", "--branch", "main",
                            "-c", "advice.detachedHead=false",
                            "https://github.com/bresilla/echo.git", sourcedir})
        end
    end)
...
if has_config("examples") and os.projectdir() == os.curdir() then
    for _, filepath in ipairs(os.files("examples/*.cpp")) do
        ...
        target(filename)
            ...
            add_files(filepath)
            add_deps("tractor")
            add_packages("concord", "agisostack", "echo")
...
if has_config("tests") and os.projectdir() == os.curdir() then
    for _, filepath in ipairs(os.files("test/*.cpp")) do
        ...
        target(filename)
            ...
            add_files(filepath)
            add_deps("tractor")
            add_packages("concord", "agisostack", "doctest", "echo")
            add_ldflags("-Wl,--start-group", "-lIsobus", "-lHardwareIntegration", "-lUtility", "-Wl,--end-group", {force = true})
            add_syslinks("pthread")
            add_includedirs("include")
